...
export default function EditCategoryPage(props) {
  ...
  const loadData = (id) => {
    ...
    try {
      HTTP.get(API.getCategoryInfo + id, false).then((res) => {
        if (res && res.status && res.status.toString() === "200") {
          if (res.data) {
            ...
            setformdata({
              ...formdata,
              name: res.data.name,
              imageurl: API.imageurl + res.data.image,
            });
            console.log(API.imageurl + "\\" + res.data.image);
            LoadingModal.hide();
          }
        } else {
          LoadingModal.hide();
  ...
  const onAddClick = () => {
    ...
    try {
      ...
      HTTP.put(API.put_category + categoryid, iData, true, false, "").then(
        (res) => {
          if (res && res.status && res.status.toString() === "200") {
            LoadingModal.hide();
            AlertModal02.show(
