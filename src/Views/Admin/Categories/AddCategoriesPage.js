...
export default function AddCategoriesPage() {
  ...
  const checkAndSetImage = (file) => {
    ...
    if (size > 51200) {
      setImage("");
      return AlertModal02.show(
        "Image size allowed upto 50KB, Please select image under 50KB",
        "Oops!",
        () => {},
        "sm"
      );
  ...
  const onAddClick = () => {
    ...
    try {
      ...
      console.log(iData.get('name'));
      HTTP.postimage(API.addCategory, iData, false, "").then((res) => {
        console.log(res);
        if (res && res.status && res.status.toString() === "200") {
          LoadingModal.hide();
          AlertModal02.show("Category saved successfully.", "Success!", () => {
            clearForm();
            FormModal.hide();
          });
        } else {
    ...
    <>
      <div className="w-100 p-2">
        <div className="d-flex-col justify-content-center">
          {/* field 01 */}
          <label htmlFor="cat_name" className="p-1 fs16 fw600 adminFont">
            Category Name
          </label>
          <GInput
            id="cat_name"
          ...
          />
          {/* field 02 */}
          <label htmlFor="cat_image" className="p-1 fs16 fw600 adminFont">
            Image
            <span className="label-msg fs-6 adminFontRegular">
              {" "}
