...
import { TeamCheckBoxStyle } from "./TeamCheckBoxStyle";
...
const TeamCheckBox = (props) => {
  ...
  const handleTeamChecked = async (event, userid) => {
    if (event.target.checked) {
      ...
      const updatedRowsState = await Promise.all(
        teams.filter((s, key) => s.teamid !== userid)
      );
        ...
        <Container>
          <Row>
            <Col md={12} className="mt20 ">
              ...
              <div className="services_pricing_h">
                <div>
                  <GCheckbox
                    name="remember"
                    id="remember"
                ...
                {Array.isArray(salonTeamList) &&
                  salonTeamList.map((team) => {
